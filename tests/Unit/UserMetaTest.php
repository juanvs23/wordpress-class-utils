...
class UserMetaTest extends TestCase
{
    ...
    // ── All field types render test ───────────────────────────────────────────
    public function test_all_field_types_render_without_error(): void
    {
        $allTypes = [
            'text'       => ['id' => 'f_text',     'type' => 'text',       'label' => 'Text'],
            'number'     => ['id' => 'f_number',   'type' => 'number',     'label' => 'Number'],
            'date'       => ['id' => 'f_date',     'type' => 'date',       'label' => 'Date'],
            'email'      => ['id' => 'f_email',    'type' => 'email',      'label' => 'Email'],
            'url'        => ['id' => 'f_url',      'type' => 'url',        'label' => 'URL'],
            'textarea'   => ['id' => 'f_textarea', 'type' => 'textarea',   'label' => 'Textarea'],
            'checkbox'   => ['id' => 'f_check',    'type' => 'checkbox',   'label' => 'Check'],
            'select'     => ['id' => 'f_select',   'type' => 'select',     'label' => 'Select',     'options' => ['a' => 'A', 'b' => 'B']],
            'media'      => ['id' => 'f_media',    'type' => 'media',      'label' => 'Media'],
            'gallery'    => ['id' => 'f_gallery',  'type' => 'gallery',    'label' => 'Gallery'],
            'list'       => ['id' => 'f_list',     'type' => 'list',       'label' => 'List'],
            'editor'     => ['id' => 'f_editor',   'type' => 'editor',     'label' => 'Editor'],
            'color'      => ['id' => 'f_color',    'type' => 'color',      'label' => 'Color'],
            'map'        => ['id' => 'f_map',      'type' => 'map',        'label' => 'Map'],
            'get_terms'  => ['id' => 'f_terms',    'type' => 'get_terms',  'label' => 'Terms',       'taxonomy' => 'category'],
            'get_posts'  => ['id' => 'f_posts',    'type' => 'get_posts',  'label' => 'Posts',       'post_type' => 'post'],
            'relationship' => ['id' => 'f_rel',    'type' => 'relationship', 'label' => 'Rel',       'post_type' => 'post'],
            'accordion'  => ['id' => 'f_acc',      'type' => 'accordion',   'label' => 'Accordion',  'fields' => [['id' => 'sub', 'type' => 'text']]],
            'repeater'   => ['id' => 'f_rep',      'type' => 'repeater',    'label' => 'Repeater',   'fields' => [['id' => 'sub', 'type' => 'text']]],
        ];
        $config = [
            'title'       => 'All Fields Test',
            'description' => 'Renders every field type',
            'fields'      => array_values($allTypes),
        ];
        $um   = new \ColtmanCreateUserMeta($config);
        $user = new \WP_User(1);
        $html = $this->capture(fn() => $um->add_user_meta_section($user));
        // Verify title and description rendered
        $this->assertStringContainsString('All Fields Test', $html);
        $this->assertStringContainsString('Renders every field type', $html);
        // Verify each field label appears
        foreach ($allTypes as $name => $field) {
            $this->assertStringContainsString(
                $field['label'],
                $html,
                "Field type '{$name}' label not found in output"
            );
        }
    }
    public function test_all_field_types_render_expected_input_elements(): void
    {
        $config = [
            'title'  => 'All Fields Render Test',
            'fields' => [
                ['id' => 'f_text',     'type' => 'text',       'label' => 'Text'],
                ['id' => 'f_number',   'type' => 'number',     'label' => 'Number'],
                ['id' => 'f_date',     'type' => 'date',       'label' => 'Date'],
                ['id' => 'f_check',    'type' => 'checkbox',   'label' => 'Check'],
                ['id' => 'f_textarea', 'type' => 'textarea',   'label' => 'TA'],
                ['id' => 'f_select',   'type' => 'select',     'label' => 'Sel',     'options' => ['a' => 'A']],
                ['id' => 'f_media',    'type' => 'media',      'label' => 'Media'],
                ['id' => 'f_gallery',  'type' => 'gallery',    'label' => 'Gallery'],
                ['id' => 'f_list',     'type' => 'list',       'label' => 'List'],
                ['id' => 'f_editor',   'type' => 'editor',     'label' => 'Editor'],
                ['id' => 'f_color',    'type' => 'color',      'label' => 'Color'],
                ['id' => 'f_map',      'type' => 'map',        'label' => 'Map'],
                ['id' => 'f_terms',    'type' => 'get_terms',  'label' => 'Terms',   'taxonomy' => 'category'],
                ['id' => 'f_posts',    'type' => 'get_posts',  'label' => 'Posts',   'post_type' => 'post'],
            ],
        ];
        $um   = new \ColtmanCreateUserMeta($config);
        $user = new \WP_User(1);
        ...
        $this->assertStringContainsString('name="f_select"',  $html, 'Select name missing');
        $this->assertStringContainsString('coltman-media',    $html, 'Media container missing');
        $this->assertStringContainsString('coltman-gallery',  $html, 'Gallery container missing');
        $this->assertStringContainsString('coltman-list',     $html, 'List container missing');
        $this->assertStringContainsString('coltman-map',      $html, 'Map container missing');
    }
}
